	...
	$.fn.isHappy = function(config) {
		...
		function processField(opts, selector) {
			...
			field.testValid = function(submit) {
				var val, el = $(this), gotFunc, error = false, temp, required = !!el.get(0).attributes.getNamedItem('required') || opts.required, password = (field.attr('type') === 'password'), arg = isFunction(opts.arg) ? opts.arg() : opts.arg;
				...
				// for some reason the built in test for blank input doesn't work, so i made this one
				if(!val && required === true)
					error = true;

				if(error) {
					el.addClass('unhappy').after(errorEl);
					el.removeClass('happy');
					return false;
				} else {
					temp = errorEl.get(0);
					// this is for zepto
