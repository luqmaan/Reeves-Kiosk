(function() {
	window.ui = {
		...
		home: function() {
			...
			$('img.currentLogoMarker').remove();
			setTimeout(function() {$("#footer #back").css("visibility", "hidden");}, 10);
            $("#content").append('<img src="images/choose-make-arrow.png" id="chooseMakeArrow" height="62" width="346" /><img id="chooseMake" src="images/_0020_CHOOSE-MAKE.png" alt="_0020_CHOOSE-MAKE" width="814" height="76"  />');


			
			// prevent the user from crashing the app when they keep switching between the video and another page
			// DISABLED: Video
			// setTimeout(launchVideo, 5000)

			function launchVideo() {
				// if online, launch the embedded video
				if (config.online) {
					// playEmbeddedVideo();
					playOfflineVideo();
				}
				// if offline, launch the fullscreen video
				else {
					playOfflineVideo();
				}
			}
			...
			function playEmbeddedVideo() {
				// if the user has changed the page, don't try to play the video again
				if (config.online && hasher.getHash() === 'home') {
					// Make the video autoplay and loop
					// The HTML5 autoplay attribute is not supported by android
					document.querySelector('video').play();
					setTimeout(playEmbeddedVideo, 22000);
				}
			}

			function playOfflineVideo() {
 
				if (hasher.getHash() === 'home') {
					window.plugins.videoPlayer.play("file:///android_asset/www/fullvideo.mp4");
					// don't  tell the window to queue the next video until the focus is brought back
					//		what if the user presses pause in the middle of the video
					$(window).one("focus", function() {
						console.log("focus!");
						setTimeout(playOfflineVideo, 30000);
					});
				}
			}
...
/*
			...
			makeMenu += "<img data-nav='s' data-make='" + make + "' data-make-name='" + data.makes[make].name + "' src='images/_0012_MODELS.png' alt='" + data.makes[make].name + " Models' class='makeMenuLink tap' />";
...
*/
		...
		loadSpecialsGallery: function() {
			...
			data.specials(make, function(tx, specials) {
				...
				if (len === 0) {
					galleryHTML += "<h2>Sorry, no offers available at this time. Please check back later!</h2>";
				}
		...
		newPage : function() {
			...
			// yes, this is very ugly but did fix the memory leak issues on android
			$("#content").off();
		...
		idleTimer: function() {
			...
			// if the user hasn't been here for idleWait time, go to the homepage and launch the screensaver
		...
		screenSaver: function() {
			...
			ui.screenSaverOn = true;
			$("#screensaver").fadeIn(1000);

			pulsateText();

			function pulsateText() {
				if (ui.screenSaverOn)
					$("#screensaver h2").effect("pulsate", { times:1 }, 2000, pulsateText);
			}


			$("#screensaver").one("click", function() {
				$("#screensaver").fadeOut(200);
				ui.screenSaverOn = false;
			});
		}
